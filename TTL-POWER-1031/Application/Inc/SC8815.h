...
#define SCHW_BATT_RSHUNT        5.15          //电池电流路径上的感测电阻值
...
#define SC8815_ADDR             0x74 << 1        //SC8815 I2C 设备地址
...
void SC8815_SetVBUSFBMode(uint8_t mode);						//设置VBUS电压反馈模式 SCHWI_FB_Internal 内部反馈， SCHWI_FB_External 外部反馈
uint8_t SC8815_GetVBUSFBMode(void);									//获取VBUS电压反馈模式
...
// 新增函数
void SC8815_SetBatteryCell(uint8_t NewCell);				//设置电池数量 			数值函义为数字1-4 表示1-4节电池
uint8_t SC8815_GetBatteryCell(void);								//获取电池数量设置 	返回值为1-4
void SC8815_SetSWFreq(uint8_t NewFreq);							//设置开关切换频率 	数值函义为 1=设置为150KHz 2=设置为转换300KHz充电150KHz 3=均设置为300KHz 4=设置为450KHz
uint8_t SC8815_GetSWFreq(void);											//获取开关切换频率 	返回值 1=设置为150KHz 2=设置为转换300KHz充电150KHz 3=均设置为300KHz 4=设置为450KHz
void SC8815_SetDeadTime(uint8_t NewDT);							//设置死区时间 			数值函义为 1=20ns 2=40ns 3=60ns 4=80ns
uint8_t SC8815_GetDeadTime(void);										//获取死区时间 			返回值函义为 1=20ns 2=40ns 3=60ns 4=80ns
void SC8815_SetILIMBW(uint8_t NewILIMBW);						//设置电流环路响应带宽 数值函义为 1=5KHz 2=1.25KHz
uint8_t SC8815_GetILIMBW(void);											//获取电流环路响应带宽 返回值 1=5KHz 2=1.25KHz
void SC8815_SetLoop(uint8_t NewLoop);								//设置环路响应模式 	数值函义为 1=正常环路响应 2=改善环路响应
